...
while IFS= read -r L; do show "$L"; done < <(
  grep -rn 'updateUI\|updateView\|refreshUI\|refreshView\|invalidate' "$APK_DIR/smali"* --include='*.smali' 2>/dev/null \
    | grep -v 'onRefresh\|SwipeRefreshLayout\|invalidate()Z\|updateUILock' || true
)
...
while IFS= read -r L; do show "$L"; done < <(
  grep -rn 'sendEmptyMessage\|sendMessageDelayed\|postDelayed\|Handler' "$APK_DIR/smali"* --include='*.smali' 2>/dev/null \
    | grep -iE 'update|check|version' || true
)
