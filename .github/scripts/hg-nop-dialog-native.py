...
for xml_file in APK.rglob("res/xml/e.xml"):
    ...
    block = """    <domain-config cleartextTrafficPermitted="false">
        ...
        <domain includeSubdomains="true">dahhxxttxs.com</domain>
        <domain includeSubdomains="true">heytapmobi.com</domain>
        <trust-anchors>
        </trust-anchors>
    </domain-config>
"""
    if block.strip() in text:
        print(f"  ✅ Already blocked all known domains")
    else:
        # Remove existing domain-config block and replace with new one
        text = re.sub(r'<domain-config.*?</domain-config>', '', text, flags=re.DOTALL)
        text = text.replace("</network-security-config>", block + "</network-security-config>")
        xml_file.write_text(text, encoding="utf-8")
        print(f"  ✅ All domains blocked")
        TOTAL += 1
    break
